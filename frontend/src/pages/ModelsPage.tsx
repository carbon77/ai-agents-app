...
function getModelTypeIcon(modelType: string) {
  if (modelType.includes("chat_completion"))
    return <TextFieldsIcon color="primary" />;
  if (modelType.includes("speech_to_text"))
    return <SpatialAudioOffIcon color="primary" />;
  if (modelType.includes("text_to_speech"))
...
export function ModelsPage() {
  ...
  const modelTypeFilters = useMemo<ModelTypeFilter[]>(
    () => [
      "all",
      ...Array.from(new Set(models.map((model) => model.model_type))),
    ],
    [models],
  );
  const filteredModels = useMemo(
    () =>
      models.filter(
        (model) =>
          (modelTypeFilter === "all" || model.model_type === modelTypeFilter) &&
      ...
      <Stack spacing={3}>
        <Box>
          ...
          <Typography color="text.secondary">
            Browse model capabilities, context windows, and supported features.
          </Typography>
        </Box>
        <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
          ...
          <ToggleButtonGroup
            ...
            onChange={(_, value: ModelTypeFilter | null) =>
              value && setModelTypeFilter(value)
            }
          ...
          >
            {modelTypeFilters.map((modelType) => (
              <ToggleButton
                key={modelType}
                value={modelType}
                aria-label={`Show ${getModelTypeLabel(modelType)} models`}
              >
        ...
        {!loading && !error && filteredModels.length === 0 && (
          <Alert severity="info">No models match your search.</Alert>
        )}
        {!loading && !error && (
          <Box
            display="grid"
          ...
          >
            {filteredModels.map((model, index) => (
              <Card
                key={`${model.model_id}-${model.model_type}-${index}`}
                elevation={0}
                sx={{
                  border: "1px solid",
                  borderColor: "divider",
                ...
                <CardContent>
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    ...
                    <Stack direction="row" gap={1} flexWrap="wrap">
                      <Chip
                        label={model.model_type.replaceAll("_", " ")}
                        size="small"
                        color="primary"
                      />
                      <Chip
                  ...
                  <Stack direction="row" gap={1} flexWrap="wrap" sx={{ mt: 2 }}>
                    <Chip
                      label={`Context: ${formatNumber(model.context_window)}`}
                      size="small"
                    />
                    <Chip
                      label={`Max output: ${formatNumber(model.max_completion_tokens)}`}
                      size="small"
                    />

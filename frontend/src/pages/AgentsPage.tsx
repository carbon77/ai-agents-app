...
function matchesSearch(agent: Agent, searchTerm: string) {
  ...
  return [agent.id, agent.name, agent.description].some((value) =>
...
function sortPinnedFirst(agents: Agent[], favoriteAgentIds: string[]) {
  ...
  return [...agents].sort(
    (first, second) =>
      Number(favoriteSet.has(second.id)) - Number(favoriteSet.has(first.id)),
  );
}
...
function loadFavoriteAgentIds() {
  try {
    ...
    return Array.isArray(storedFavorites)
      ? storedFavorites.filter((id) => typeof id === "string")
      : [];
  } catch {
    return [];
  }
    ...
    >
      {sortPinnedFirst(agents, favoriteAgentIds).map((agent) => (
        <AgentCard
          key={agent.id}
          agent={agent}
          favorite={favoriteAgentIds.includes(agent.id)}
...
export function AgentsPage() {
  ...
  const [favoriteAgentIds, setFavoriteAgentIds] =
    useState<string[]>(loadFavoriteAgentIds);
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string>();

  const allAgents = useMemo(
    () => sections.flatMap((section) => section.agents),
    [sections],
  );
  const filteredAgents = useMemo(
    () => allAgents.filter((agent) => matchesSearch(agent, searchTerm)),
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      JSON.stringify(favoriteAgentIds),
    );
  }, [favoriteAgentIds]);
        ...
        >
          <Box>
            ...
            <Typography color="text.secondary">
              Search, pin favorites, and choose an agent to start a focused
              conversation.
            </Typography>
          </Box>
          <ToggleButtonGroup
          ...
          >
            <ToggleButton value="sections" aria-label="Show subsections">
              <ViewAgendaIcon />
            </ToggleButton>
            <ToggleButton value="all" aria-label="Show all agents">
              <GridViewIcon />
            </ToggleButton>
          </ToggleButtonGroup>
        </Stack>

...
async def stream_agent_event(kind: str, item: any) -> AsyncIterable[ServerSentEvent]:
    try:
        if kind == "messages":
            ...
            yield sse_event("tool_call_start", {"call": call})
            await asyncio.sleep(2)
            for delta in item.output_deltas:
                yield sse_event("tool_call_delta", {"delta": delta})
            yield sse_event("tool_call_end", {"call": call, "output": item.output, "error": item.error})
